...
def parse_args():
    ...
    parser.add_argument(
        "--paragraph_file",
        type=str,
        default="/kaggle/input/ntu-adl-2025-hw-1/context.json",
    ...
    parser.add_argument(
        "--train_file", type=str, default="/kaggle/input/ntu-adl-2025-hw-1/train.json", help="A csv or a json file containing the training data."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--validation_file", type=str, default="/kaggle/input/ntu-adl-2025-hw-1/valid.json", help="A csv or a json file containing the validation data."
    )
    parser.add_argument(
        "--test_file", type=str, default="/kaggle/input/ntu-adl-2025-hw-1/test.json", help="A csv or a json file containing the Prediction data."
    )
    parser.add_argument(
        "--max_seq_length",
        type=int,
    ...
    parser.add_argument("--output_dir", type=str, default="/kaggle/working/", help="Where to store the final model.")
...
def main():
    ...
    def prepare_train_features(examples):
        ...
        for i, offsets in enumerate(offset_mapping):
            ...
            # print("si: " + str(sample_index))
            # paragraph = context[examples[context_column_name][sample_index]]
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            if step_count % int(args.max_train_steps / 29) == 0 and step_count > 0:
                # Evaluation
                logger.info("***** Running Evaluation (validation set)*****")
                # logger.info(f"  Num examples = {len(eval_dataset)}")
                # logger.info(f"  Batch size = {args.per_device_eval_batch_size}")
                valid_loss = 0
                print("Now steps: " + str(step_count))
                ...
                logger.info(f"Evaluation metrics (V): {eval_metric}")
                
                if args.with_tracking:
                    log = {
                        "train_loss": total_loss.item() / step_count,
                        "valid_loss": valid_loss.item() / len(eval_dataloader),
                        # "train_exact_match": train_metric["exact_match"],
                        "valid_exact_match": eval_metric["exact_match"],
                        "epoch": epoch,
